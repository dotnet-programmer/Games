...
internal class Game
{
	...
	private const int PlayfieldWidth = 200;
	private const int PlayfieldHeight = 400;
	...
	public Game(Rectangle playfieldBackground)
	{
		...
		_playfield = new(
			_playfieldBackground.Right / 2 - PlayfieldWidth / 2,
			_playfieldBackground.Bottom / 2 - PlayfieldHeight / 2,
			PlayfieldWidth,
			PlayfieldHeight);
	...
	private Shape GetNewShape(int x, int y)
		=> new((ShapeKind)_random.Next(ShapeCount), x, y);

	private Shape GetNewShape()
	{
		//ShapeKind shapeKind = ShapeKind.J;
		ShapeKind shapeKind = (ShapeKind)_random.Next(ShapeCount);
		return shapeKind switch
		{
			ShapeKind.I => new(shapeKind, _nextSegmentArea.Left + _nextSegmentArea.Width / 2, _nextSegmentArea.Top + _nextSegmentArea.Height / 2 - SegmentSize / 2),
			ShapeKind.J => new(shapeKind, _nextSegmentArea.Left + _nextSegmentArea.Width / 2 - SegmentSize / 2, _nextSegmentArea.Top + _nextSegmentArea.Height / 2 - SegmentSize),
			ShapeKind.L => new(shapeKind, _nextSegmentArea.Left + _nextSegmentArea.Width / 2 - SegmentSize / 2, _nextSegmentArea.Top + _nextSegmentArea.Height / 2 - SegmentSize),
			ShapeKind.O => new(shapeKind, _nextSegmentArea.Left + _nextSegmentArea.Width / 2, _nextSegmentArea.Top + _nextSegmentArea.Height / 2 - SegmentSize),
			ShapeKind.S => new(shapeKind, _nextSegmentArea.Left + _nextSegmentArea.Width / 2 - SegmentSize / 2, _nextSegmentArea.Top + _nextSegmentArea.Height / 2 - SegmentSize),
			ShapeKind.T => new(shapeKind, _nextSegmentArea.Left + _nextSegmentArea.Width / 2 - SegmentSize / 2, _nextSegmentArea.Top + _nextSegmentArea.Height / 2 - SegmentSize),
			ShapeKind.Z => new(shapeKind, _nextSegmentArea.Left + _nextSegmentArea.Width / 2 - SegmentSize / 2, _nextSegmentArea.Top + _nextSegmentArea.Height / 2 - SegmentSize),
	...
	public void DoAction(Keys keyCode)
	{
	...
	private void MoveLeft()
	{
		...
		{
			Point point = new(_actualShape.Segments[i].Location.X - SegmentSize, _actualShape.Segments[i].Location.Y);
	...
	private void MoveRight()
	{
		...
		{
			Point point = new(item.Location.X + SegmentSize, item.Location.Y);
		...
		if (_actualShape.MaxPosX < _playfield.Right - SegmentSize)
		{
	...
	private void MoveDown()
	{
		for (int i = 0; i < _actualShape!.Segments.Count; i++)
		{
			Point point = new(_actualShape.Segments[i].Location.X, _actualShape.Segments[i].Location.Y + SegmentSize);
			for (int j = 0; j < _segments.Count; j++)
			{
				if (point == _segments[j].Location)
				{
		...
		if (_actualShape.MaxPosY + SegmentSize == _playfield.Bottom)
		{
			_segments.AddRange(_actualShape.Segments);
			_actualShape = null;
			CheckLines();
	...
	{
		int maxSegmentsInRow = PlayfieldWidth / SegmentSize;

		for (int i = _segments.Min(s => s.Location.Y); i <= _segments.Max(s => s.Location.Y); i += SegmentSize)
		{
			if (maxSegmentsInRow == _segments.Count(s => s.Location.Y == i))
			{
				...
				tmpList.ForEach(s => s.Location = new Point(s.Location.X, s.Location.Y + SegmentSize));
